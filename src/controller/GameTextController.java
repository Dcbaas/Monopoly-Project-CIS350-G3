...
public class GameTextController {
  ...
  public GameTextController(Game game, GameTextView view, boolean canRoll, boolean canBuy,
      boolean canMortgage, boolean canDraw) {
    this.game = game;
    this.view = view;
  ...
  private void draw() {
    ...
    String deckType = game.getBoard().getSquaresList().get(game.getCurrentPlayer().getPosition())
        .getName();
    ...
    try {
      ...
      System.out.println(e + "could not find card");
    }
    canDraw = false;
  }
      ...
      if (game.getCurrentPlayer().getGroupsOwned().size() > 0 && game.getCurrentPlayer()
        ...
        if (property.isHasHotel() == false) {
          ...
          if (property.getNumHouses() < 4) {
            ...
            // If you want to build the second house on a property, all other props in that group
            // need to have at least 1 house
  ...
  private void checkDraw() {
    ...
    if (game.getBoard().getLocationType(game.getCurrentPlayer().getPosition()) == 2
        && (game.getBoard().getSquaresList().get(game.getCurrentPlayer().getPosition()).getName()
            .equals("CHANCE")
            || game.getBoard().getSquaresList().get(game.getCurrentPlayer().getPosition()).getName()
