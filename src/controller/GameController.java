...
/**
 * This is the main game controller witch handles all the engine
 * logic between the view and the game.
 *
...
public class GameController {
  ...
  /**
   * Tracks the number of pairs the user gets.
   */
  private int numPairs;

  /**
   * Tracks if the user can roll.
   */
  private boolean canRoll;

  /**
   * Tracks if the user  can buy.
   */
  private boolean canBuy;

  /**
   * Tracks if the user can mortgage.
   */
  private boolean canMortgage;

  /**
   * Tracks if the user can draw.
   */
  private boolean canDraw;

  /**
   * Tracks if the user is in testing mode.
   */
   ...
   */
  public GameController(Game game, GameView view, boolean canRoll,
  ...
  public void commands(String command) throws InterruptedException {
    ...
    if (numPairs == 3) {
      canRoll = false;
      canBuy = false;
      view.getGamePanel().movePlayer(game.getCurrentPlayer().getPosition(), game.sendPlayerToJail(),
  ...
  private void mortgage() throws InterruptedException {
    ...
    while (run) {
      ...
      command = JOptionPane.showInputDialog(
          "Type The name of the property you would like to mortgage");
      ...
      for (OwnableSquare propertyOwned : game.getCurrentPlayer()
          .getPropertiesOwned()) {
      ...
      if (game.getCurrentPlayer().getPropertiesOwned()
          .contains(property.getGROUP_NUMBER())) {
        //The array of the specific group of properties the player owns
        ArrayList<PropertySquare> group = (ArrayList<PropertySquare>) game
            .getBoard()
        ...
        group = group.stream()
            .filter(groupProperty -> groupProperty.getNumHouses() > 0
                || groupProperty.isHasHotel())
            .collect(
        ...
        if (!group.isEmpty()) {
          ...
          while (!answered) {
            ...
            command = JOptionPane.showInputDialog(
                "Would you like to sell all building for the group matching the \n property t"
                    + "hat is going to be mortgaged ? (YES/NO) \n or type  done to stop action");
            ...
            if (command.equalsIgnoreCase("yes") || command
                .equalsIgnoreCase("no")) {
              ...
              if (command.equalsIgnoreCase("yes")) {
                ...
                sellBuildings(group);
                game.getCurrentPlayer()
                    .removeGroupOwned(group.get(0).getGROUP_NUMBER());
                group = null;
              }
            } else {
              view.getTextPanel().printToTextArea(
          ...
          if (group == null) {
            ...
            view.getTextPanel().printToTextArea(
                property.getName() + " was mortgaged for " + property
                    .getMORTGAGE_VAL());
          } else {
            view.getTextPanel().printToTextArea(
                "In order to mortgage the property, " + game.getCurrentPlayer()
                    .getDisplayName()
                    + " must sell all the buildings \n for all properties in that group.");
          }
        }
      } else {
  ...
  private void roll() {
    if (testing) {
      ...
      JOptionPane
          .showConfirmDialog(null, message, "Enter dice values", JOptionPane.OK_CANCEL_OPTION);
    ...
    // Check if player is not in jail OR player is in Jail AND doubles were rolled OR
    // player is holding get out of jail free card
    if (!checkJail(game.getCurrentPlayer()) || (checkJail(game.getCurrentPlayer())
        && game.getDieOne().getValue() == game.getDieTwo().getValue()) || (
        checkJail(game.getCurrentPlayer()) && game.getCurrentPlayer().getCardsHeld().size() > 0)) {

      if (checkJail(game.getCurrentPlayer()) && game.getCurrentPlayer().getCardsHeld().size() > 0) {
        game.getCurrentPlayer().setInJail(-1);
        view.getTextPanel().printToTextArea(game.getCurrentPlayer().getDisplayName()
            + " was in Jail but used a Get Out Of Jail Free card.");
        game.getCurrentPlayer().getCardsHeld().remove(0);
      }

      if (checkJail(game.getCurrentPlayer()) && game.getDieOne().getValue() == game.getDieTwo()
          .getValue()) {
        game.getCurrentPlayer().setInJail(-1);
        view.getTextPanel().printToTextArea(game.getCurrentPlayer().getDisplayName()
            + " was in Jail but rolled doubles so is now free at no cost.");
      }
      ...
      if (game.getCurrentPlayer().getPosition() == 30) {
        view.getTextPanel().printToTextArea(game.getCurrentPlayer().getDisplayName()
            + " landed on Go To Jail and was sent to Jail.");
      ...
      view.getTextPanel().printToTextArea(
          game.getCurrentPlayer().getDisplayName() + " is in Jail and cannot move for " + num
              + " more turns.");
  ...
  private void buy() {
    ...
    if (game.checkIfOwnable(game.getCurrentPlayerLocation()) != null
      ...
      if (game.buyOwnableSquare((OwnableSquare) game.getCurrentPlayerLocation(),
        ...
        view.getTextPanel().printToTextArea(
            game.getCurrentPlayer().getDisplayName() + " bought " + game
                .getCurrentPlayerLocation().getName());
      } else {
      ...
      view.getTextPanel()
          .printToTextArea("This property cannot be bought at the moment.");
    }
  ...
  private void draw() {
    ...
    String deckType = game.getBoard().getSquaresList()
        .get(game.getCurrentPlayer().getPosition()).getName();

    final int position = game.getCurrentPlayer().getPosition();
    ...
    try {
      view.getTextPanel().printToTextArea(
          game.getCurrentPlayer().getDisplayName() + " drew:\n\t" + card
              .getCardDescription());
    ...
    try {
      if (card.getCardDescription().equals("Go to Jail - Go directly to jail")) {
        view.getGamePanel()
            .movePlayer(game.getCurrentPlayer().getPosition(), game.sendPlayerToJail(),
                game.getCurrentPlayer());
      }
    } catch (NullPointerException e) {
      System.out.println("action couls not be performed (end player to jail)");
    }
    view.getButtonPanel().toggleButtons();
  }
  ...
  private void tax() {
    view.getTextPanel().printToTextArea(
        game.getCurrentPlayer().getDisplayName() + " paid " + game.collectFee()
            + " in fees/rent");
  }
  ...
  private void buildHouse() throws InterruptedException {
    ...
    String propertyName = JOptionPane
        .showInputDialog("Enter which property you want to build on below.");
    ...
    if (game.getBank().getNumHouses() >= 1) {
      ...
      if (game.getCurrentPlayer().getGroupsOwned().size() > 0 && game
          .getCurrentPlayer()
        ...
        if (property.isHasHotel() == false) {
          ...
          if (property.getNumHouses() < 4) {
            ...
            // If you want to build the second house on a property, all other props in that
            // group need to have at least 1 house
            if (checkHouseCount(property) == true) {
              // Check that player has enough money
              if (game.getCurrentPlayer().getWallet() >= property
                  .getHouseCost()) {
                //add one house to the desired property
                property.setNumHouses(property.getNumHouses() + 1);
                //Update the view.
                view.getGamePanel()
                    .setHouses(property.getPOSITION(), property.getNumHouses());
                ...
                // Print build success
                view.getTextPanel().printToTextArea(
                    "A house was successfully built on " + property.getName()
                        + ".");
              } else {
                view.getTextPanel().printToTextArea(
                    game.getCurrentPlayer().getDisplayName()
                        + " does not have enough money to buy this");
              }
            } else {
              view.getTextPanel().printToTextArea(
                  "House counts across this group are not even. Try building on a different "
                      + "property in this group first.");
            }
          } else {
            view.getTextPanel().printToTextArea(
                "Property already has 4 houses. Build a hotel here.");
          }
        } else {
          ...
          if (property.getNumHouses() == 4) {
            ...
            for (BoardSquare square : game.getBoard().getSquaresList()) {
              ...
              if (isHouseCountEven == true) {
                ...
                if (square.getType() == 0) {
                  ...
                  if (propertySquare.getGROUP_NUMBER() == property
                      .getGROUP_NUMBER()) {
                    // Make sure num houses is not less desired prop num houses - 1
                    if (propertySquare.getNumHouses() < 4
                        && propertySquare.isHasHotel() == false) {
            ...
            if (isHouseCountEven == true) {
              ...
              if (game.getCurrentPlayer().getWallet() >= property
                ...
                // Print build success
                view.getTextPanel().printToTextArea(
                    "A hotel was successfully built on " + property.getName()
                        + ".");
              } else {
                view.getTextPanel().printToTextArea(
                    game.getCurrentPlayer().getDisplayName()
            ...
            view.getTextPanel().printToTextArea(
                "Cannot build a Hotel. This property does not have 4 houses yet.");
          }
        } else {
          view.getTextPanel().printToTextArea("This property has a hotel.");
        }
  ...
  private void checkDraw() {
    ...
    if (game.getBoard().getLocationType(game.getCurrentPlayer().getPosition())
        == 2
        &&
        (game.getBoard().getSquaresList()
            .get(game.getCurrentPlayer().getPosition()).getName()
   ...
   *
   * @return wheteher the player is testing or not.
   */
   ...
   *
   * @param testing the boolean that contains the state of the testing.
   */
